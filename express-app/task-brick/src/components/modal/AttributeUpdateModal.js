...
const AttributeUpdateModal = ({ open, onClose, attribute, selectedIssue }) => {
    ...
    <div className={`fixed inset-0 z-50 ${open ? '' : 'hidden'} bg-black bg-opacity-50 flex items-center justify-center p-4`}>
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        {attribute === 'assigneeId' && (
            ...
            {users.map(user => (
                <option key={user._id} value={user._id}>{user.firstName}{user.lastName}</option> // Adjust according to your user object structure
            ))}
            </select>
        )}
